...
void TestPositionAndStringConversion() {
  ...
  testSingle(Position{Position::MAX_ROWS - 1, Position::MAX_COLS - 1},
             "XFD16384");
}
...
ParseFormula(std::move(expr))->GetExpression(); };
...
void TestFormulaReferencedCells() {
  ...
  // auto xx = tricky->GetReferencedCells();
...
void TestCellReferences() {
  ...
  ASSERT_EQUAL(sheet->GetCell("A2"_pos)->GetReferencedCells(), std::vector
{"A1"_pos}); ASSERT_EQUAL(sheet->GetCell("B2"_pos)->GetReferencedCells(),
std::vector {"A1"_pos});
...
{"B1"_pos});
  ...
  ASSERT_EQUAL(sheet->GetCell("B1"_pos)->GetReferencedCells(), std::vector
{"C3"_pos});
}
